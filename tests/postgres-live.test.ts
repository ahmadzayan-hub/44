...
import { PgAuditLog, PgMemoryStore, PgReportStore, ensureSchema } from '../src/persistence/postgres.ts';
...
test('live PostgreSQL adapters (requires DATABASE_URL)', { skip: url ? false : 'DATABASE_URL not set' }, async () => {
  ...
  try {
    await db.query('DROP TABLE IF EXISTS railmind_audit_events, railmind_memory_records, railmind_reports');
